...
def test_telegram_sender_raises_nonzero_exit(monkeypatch, tmp_path, caplog):
    fake_script = tmp_path / "safe_telegram_send.sh"
    fake_script.write_text("#!/bin/sh\nexit 1\n")
    fake_script.chmod(0o755)
    monkeypatch.setenv("PAPERCLIP_NOTIFY_SENDER", str(fake_script))
    ...
    with caplog.at_level("ERROR"):
        try:
            sender("msg")
        except paperclip_notify_server.NotifyDeliveryError as exc:
            assert "rc=7" in str(exc)
        else:
            raise AssertionError("expected NotifyDeliveryError")
...
def test_telegram_sender_raises_timeout(monkeypatch, tmp_path, caplog):
    fake_script = tmp_path / "safe_telegram_send.sh"
    fake_script.write_text("#!/bin/sh\nsleep 60\n")
    fake_script.chmod(0o755)
    monkeypatch.setenv("PAPERCLIP_NOTIFY_SENDER", str(fake_script))
...
def test_build_app_returns_502_when_sender_fails(monkeypatch, tmp_path):
    monkeypatch.setenv("PAPERCLIP_NOTIFY_TOKEN", "tkn")
    monkeypatch.setenv("PAPERCLIP_NOTIFY_DB", str(tmp_path / "d.db"))
    fake_script = tmp_path / "safe_telegram_send.sh"
    fake_script.write_text("#!/bin/sh\nexit 1\n")
    ...
    monkeypatch.setattr(subprocess, "run", fake_run)
    app = paperclip_notify_server.build_app(target="128314698")
    c = TestClient(app)
    payload = {
        "check": "x",
        "status": "warn",
    ...
    r1 = c.post("/paperclip/notify", json=payload, headers={"Authorization": "Bearer tkn"})
    r2 = c.post("/paperclip/notify", json=payload, headers={"Authorization": "Bearer tkn"})
