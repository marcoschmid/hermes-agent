...
class TestTryPaymentFallback:
    ...
    def test_skips_to_codex_when_or_and_nous_fail(self):
        ...
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_nous", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_custom_endpoint", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_codex", return_value=(mock_codex, "gpt-5.4-mini")), \
             patch("agent.auxiliary_client._read_main_provider", return_value="openrouter"):
            client, model, label = _try_payment_fallback("openrouter")
...
class TestAuxiliaryAuthRefreshRetry:
    def test_call_llm_refreshes_codex_on_401_for_vision(self):
        ...
        with (
            patch(
                ...
                side_effect=[("openai-codex", failing_client, "gpt-5.4-mini"), ("openai-codex", fresh_client, "gpt-5.4-mini")],
            ),
    ...
    def test_call_llm_refreshes_codex_on_401_for_non_vision(self):
        ...
        with (
            patch("agent.auxiliary_client._resolve_task_provider_model", return_value=("openai-codex", "gpt-5.4-mini", None, None, None)),
            patch("agent.auxiliary_client._get_cached_client", side_effect=[(stale_client, "gpt-5.4-mini"), (fresh_client, "gpt-5.4-mini")]),
            patch("agent.auxiliary_client._refresh_provider_credentials", return_value=True) as mock_refresh,
        ):
            resp = call_llm(
                task="compression",
