...
def _write_json(path: Path, data: dict) -> None:
    ...
    Combines both hardening passes:
    - secure_parent_dir tightens the parent dir to 0o700 so siblings can't
      traverse to the creds (refuses to chmod / or top-level dirs, #25821).
    - A UNIQUE per-write temp via mkstemp (kernel-atomic O_EXCL create at
      0o600) closes the umask TOCTOU window that ``write_text`` + post-chmod
      had (#19673) AND the self-unlink/collision a fixed-name shared temp
      suffers under concurrent writers for the same token path. os.replace
      swaps it in atomically; fsync makes the write durable.
    """
