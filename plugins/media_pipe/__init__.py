...
MEDIA_PIPE_CLI = os.environ.get(
    "MEDIA_PIPE_CLI",
    "/Users/marco/.openclaw/workspace/scripts/media-pipe",
)

PROVIDER_SPECIFIC_FIELDS = frozenset(
    {"provider", "provider_id", "model", "tool", "adapter", "candidate_calls"}
)


def _available() -> bool:
    return Path(MEDIA_PIPE_CLI).exists() and os.access(MEDIA_PIPE_CLI, os.X_OK)
...
def _run_cli(argv: list[str]) -> str:
    ...
    if proc.returncode != 0:
        return _json_response(
            {
                "success": False,
                "error": (proc.stderr or proc.stdout or "media-pipe failed").strip(),
    ...
    try:
        ...
        return _json_response({"success": True, "stdout": proc.stdout})
    if isinstance(parsed, dict):
        parsed.setdefault("success", True)
        return _json_response(parsed)
    return _json_response({"success": True, "result": parsed})
...
def _request_from_args(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if bad:
        raise ValueError(
            "Media Pipe requests must stay provider-neutral; remove: "
            + ", ".join(bad)
        )
    return request
...
def _paperclip_sync_enabled(args: dict[str, Any]) -> bool:
    if bool(args.get("paperclip_sync")):
        return True
    return bool(
        os.environ.get("PAPERCLIP_ISSUE_ID")
        or os.environ.get("PAPERCLIP_COMPANY_ID")
    )
...
def _handle_quote(args: dict[str, Any], **_kw: Any) -> str:
    try:
        ...
        return _json_response(
            {"success": False, "error": str(exc), "error_type": "invalid_request"}
        )
    ...
    try:
        ...
        _append_optional(
            argv,
            "--paperclip-issue-id",
            args.get("paperclip_issue_id") or os.environ.get("PAPERCLIP_ISSUE_ID"),
        )
        _append_optional(
            argv,
            "--paperclip-company-id",
            args.get("paperclip_company_id")
            or os.environ.get("PAPERCLIP_COMPANY_ID"),
        )
        _append_optional(
            argv,
            "--paperclip-agent-id",
            args.get("paperclip_agent_id") or os.environ.get("PAPERCLIP_AGENT_ID"),
        )
...
def _handle_status(args: dict[str, Any], **_kw: Any) -> str:
    identifier = str(
        args.get("id") or args.get("run_id") or args.get("quote_id") or ""
    ).strip()
    if not identifier:
...
def _handle_ledger(args: dict[str, Any], **_kw: Any) -> str:
    if not args.get("today", True):
        return _json_response(
            {"success": False, "error": "Only today=true is supported"}
        )
    return _run_cli(["ledger", "--today"])


MEDIA_PIPE_QUOTE_SCHEMA: dict[str, Any] = {
    "type": "object",
    "properties": {
        "request": {
            "type": "object",
        ...
        "request_json": {
            "type": "string",
            "description": "Provider-neutral MediaRequest JSON string.",
        },
        "prompt": {
            "type": "string",
            "description": "Convenience brief when no request object is supplied.",
        },
        "intent": {
            "type": "string",
...
MEDIA_PIPE_RUN_SCHEMA: dict[str, Any] = {
    "type": "object",
    "properties": {"quote_id": {"type": "string"}},
    "required": ["quote_id"],
}

MEDIA_PIPE_STATUS_SCHEMA: dict[str, Any] = {
    "type": "object",
    "properties": {
        "id": {"type": "string"},
...
MEDIA_PIPE_LEDGER_SCHEMA: dict[str, Any] = {
    "type": "object",
...
def register(ctx: Any) -> None:
    ctx.register_tool(
        ...
        requires_env=[],
        description=(
            "Create a provider-neutral Media Pipe quote. The agent supplies "
            "intent, workflow, brief, constraints, and strategy; providers/"
            "models are chosen by Media Pipe."
        ),
        emoji="MP",
    )
    ctx.register_tool(
        ...
        requires_env=[],
        description=(
            "Run an approved Media Pipe quote. Approval can be local or "
            "linked Paperclip approval."
        ),
        emoji="MP",
    )
    ctx.register_tool(
